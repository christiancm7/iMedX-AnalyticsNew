...
async function getListBasemap() {
  let res = await fetch(ip_gis + 'gis/get_basemap').then((resp) => {
    if (!resp.ok) throw Error('There is something wrong');
    return resp.json();
  });
  return res;
}
...
function refreshPanel() {
  ...
  basemap_options.forEach((name) => {
    ...
    if (name === 'None')
      node.innerHTML =
        '<input type="radio" class="thumbnil" name="basemap" value="' +
        name +
        '" checked/>';
    else
      node.innerHTML =
    ...
    node.innerHTML +=
      '<img class="img-thumbnail" style="color:white;width:100px;height:70px;" src="' +
      ip_gis +
      'static/uploads/basemap/' +
      name +
      '.jpg" alt="' +
      name +
      '">' +
      '<div style="word-break: break-word;max-width: 100px;font-size:13px">' +
      name +
      '</div>';
...
function loadDrawing() {
  ...
  call.then((val) => {
    if (val.status !== 'OK') {
      drawList.innerHTML =
        '<tr><td colspan="3">There is an error while fetching data</td></tr>';
    ...
    if (id) {
      ...
      get_feature_details(id).then(function (response) {
        ...
        Object.keys(info[4].value).forEach((i) => {
          html +=
            '                    <div class="checkbox" style="margin-bottom:unset">' +
            '                            <label>';
...
async function get_feature_details(id) {
  let res = await fetch(ip_gis + 'gis/get_feature_details?id=' + id).then(
    function (response) {
      if (!response.ok) {
...
function refreshDrawingList(data) {
  ...
  data.forEach((val, idx) => {
    html +=
      ...
      '<i class="lnr-map"></i>' +
      '</button>' +
      '<button class="btn btn-icon btn-info mr-2" title="Edit Details">' +
  ...
  $('tbody[id="listDrawing"]  > tr > td > div > input[type="checkbox"]').change(
    (e) => {
      let id = e.target.id;
      let get_check = $('#' + id).prop('checked');
      fetch(ip_gis + 'gis/get_draw_features?id=' + id)
        .then((resp) => {
          if (!resp.ok) throw Error('There is something wrong');
          return resp.json();
        })
          ...
          if (get_check) {
            ...
            collection[id].getArray().forEach(function (feat) {
              ...
              if (features.a.length > 0) {
                let idx = features.a.findIndex(
                  (item) => item.getId() === l.getId()
                );
                if (idx > -1) {
  ...
  $('tbody[id="listDrawing"] > tr > td > button[title="Delete"]').click((e) => {
    ...
    createModal();
    $(
      '#gisModal .modal .modal-dialog .modal-content .modal-header .modal-title'
    )[0].innerText = 'Delete Confirmation';
    $('#gisModalOk')[0].innerText = 'Yes';

    let html = 'Are you sure you want to delete the selected item?';

    $(
      '#gisModal .modal .modal-dialog .modal-content .modal-body'
    )[0].innerHTML = html;
    ...
    $('#gisModalOk').click((e) => {
      ...
      fetch(ip_gis + 'gis/delete_draw/' + id, config)
        .then((resp) => {
          if (!resp.ok) throw Error('There is something wrong');
          return resp.json();
        })
  ...
  $(
    'tbody[id="listDrawing"] > tr > td > button[title="Modify Features"]'
  ).click((e) => {
    let target = e.currentTarget;
    let id = $(target).parents()[1].children[0].children[0].children[0].id;
  ...
  $('tbody[id="listDrawing"] > tr > td > button[title="Edit Details"]').click(
    (e) => {
      let target = e.currentTarget;
      let id = $(target).parents()[1].children[0].children[0].children[0].id;
      ...
      $(
        '#gisModal .modal .modal-dialog .modal-content .modal-header .modal-title'
      )[0].innerText = 'Edit Virtual Fence';
      $('#gisModalOk')[0].innerText = 'Edit';

      fetch(ip_gis + 'gis/get_draw_details?id=' + id)
        .then((response) => {
          if (!response.ok) throw Error('There is something wrong');
          return response.json();
        })
        .then((result) => {
          let html =
            ...
            '<input type="text" id="opstart" name="operation_start" value="' +
            result.operation_start +
            '" class="form-control" placeholder="Type here" required>' +
            '</div>' +
            '<div class="form-group">' +
            '<label for="opstop">Operation Stop</label>' +
            '<input type="text" id="opstop" name="operation_stop" value="' +
            result.operation_stop +
          ...
          $('#Bot Tracker').attr(
            'checked',
            result.trigger_by['Bot Tracker'] === 'Yes'
          );
          $('#Target').attr('checked', result.trigger_by['Target'] === 'Yes');
          $('#UnknownRadar').attr(
            'checked',
            result.trigger_by['Unknown (Radar)'] === 'Yes'
          );
        });
      ...
      $('#gisModalOk').click((e) => {
        ...
        fetch(ip_gis + 'gis/save_drawing', config)
          .then((response) => {
            if (!response.ok) throw Error('There is something wrong');
            return response.json();
          })
...
document.getElementById('saveDrawingBtn').addEventListener('click', () => {
  ...
  drawingSource.getFeatures().forEach(function (feat) {
    ...
    if (geom === 'C') obj['type'] = 'Point';
    else if (geom === 'M') obj['type'] = 'LineString';
    else if (geom === 'D') obj['type'] = 'Polygon';
...
function saveDrawing(data) {
  ...
  $('#gisModalOk').click((e) => {
    ...
    raw.forEach((val) => {
      if (trigger_by.includes(val.name))
        details['trigger_by'][val.name] = val.value;
      else if (val.name === 'status' && val.value === 'on') val.value = true;
...
function modify_drawing(id) {
  ...
  fetch(ip_gis + 'gis/save_drawing/' + id, config)
    .then((response) => {
      if (!response.ok) throw Error('There is something wrong');
      return response.json();
    })
...
function measureEnd(evt) {
  if (evt.feature.getGeometry().constructor.name === 'C') {
    ...
    let convert = new ol.proj.transform(
      [latlong[0], latlong[1]],
      'EPSG:3857',
      'EPSG:4326'
    );
    measureTooltipElement.innerHTML =
      convert[0].toFixed(6) + ', ' + convert[1].toFixed(6);
...
function loadBookmark() {
  bookmarkList.innerHTML =
    '<tr><td colspan="2">List is fetching data...</td></tr>';
  let call = getListBookmark();
  call.then((val) => {
    if (val.status !== 'OK') {
      bookmarkList.innerHTML =
        '<tr><td colspan="2">There is an error while fetching data</td></tr>';
      throw Error('Status returned is not OK');
    }
...
function refreshBookmarkList(data) {
  ...
  data.forEach((val, idx) => {
    html +=
      '<tr>' +
      '<td><span style="cursor: pointer" class="ml-3" id="' +
      val.id +
      '">' +
      val.name +
      '</span></td>' +
      '<td style="text-align: center">' +
      '<button class="btn btn-icon btn-info mr-2" title="Edit">' +
      '<i class="lnr-pencil"></i>' +
      '</button>' +
      '<button class="btn btn-icon btn-danger mr-2" title="Delete">' +
      '<i class="lnr-trash"></i>' +
  ...
  $('tbody[id="listBookmark"] > tr > td > button[title="Delete"]').click(
    (e) => {
      ...
      fetch(ip_gis + 'gis/delete_bookmark/' + id, config)
  ...
  $('tbody[id="listBookmark"] > tr > td > button[title="Edit"]').click((e) => {
    ...
    if (target.innerHTML === '<i class="lnr-pencil"></i>') {
      ...
      fetch(ip_gis + 'gis/edit_bookmark', config)
        .then(function (response) {
          if (!response.ok) throw Error('There is something wrong');
          return response.json();
        })
...
document.getElementById('addBookmark').addEventListener('click', () => {
  ...
  fetch(ip_gis + 'gis/add_bookmark', config)
    .then(function (response) {
      if (!response.ok) throw Error('There is something wrong');
      return response.json();
    })
...
$('#oltree')
  .jstree({
    ...
    contextmenu: {
      ...
      items: function (node) {
        if (
          ['Layer', 'AIS', 'Bot Tracker', 'Unknown (Radar)'].includes(
            node.text
          ) === false
        ) {
          return {
  ...
  if (['2', '3', '4', '5'].includes(data.node.id)) {
    ...
    map.getLayers().forEach(function (v, i) {
      if (v.get('id') === data.node.id) {
        ...
        if ($('#oltree').jstree().get_node(v.get('id'))) v.set('visible', true);
      }
    });
  }
}
...
$('#addBasemapBtn').click(() => {
  ...
  $('#gisModalOk').click((e) => {
    ...
    console.log(fd);
    fetch(ip_gis + 'gis/add_basemap', config)
      .then((response) => {
        if (!response.ok) throw Error('There is something wrong');
        return response.json();
      })
...
$('#deleteBasemapBtn').click(() => {
  ...
  $('#gisModalOk').click((e) => {
    ...
    fetch(ip_gis + 'gis/delete_basemap/' + basemap, config)
      .then((response) => {
        if (!response.ok) throw Error('There is something wrong');
        return response.json();
      })
        ...
        if (node.type === 'layer') {
          return {
            editLayer: {
              ...
              action: function (obj) {
                let parent;
                fetch(ip_gis + 'gis/get_layer?id=' + node.id)
                  .then((response) => {
                    if (!response.ok) throw Error('There is something wrong');
                    return response.json();
                  })
                  .then((result) => {
                    ...
                    let html =
                      ...
                      result.name +
                      '" name="name" class="form-control" placeholder="Type here" required>' +
                      '</div>' +
                      '<div class="form-group">' +
                      '<label for="type">Type</label>' +
                      '<input type="text" id="type" value="' +
                      result.type +
                      '" name="type" class="form-control" placeholder="Type here" required>' +
                      '</div>' +
                      '<div class="form-group">' +
                      '<label for="url">URL</label>' +
                      '<input type="text" id="url" value="' +
                      result.url +
                      '" name="url" class="form-control" placeholder="Type here" required>' +
                      '</div>' +
                      '<div class="form-group">' +
                      '<label for="imagerySet">ImagerySet</label>' +
                      '<input type="text" id="imagerySet" value="' +
                      result.imagerySet +
                      '" name="imagerySet" class="form-control" placeholder="Type here">' +
                      '</div>' +
                      '<div class="form-group">' +
                      '<label for="layer">Layer</label>' +
                      '<input type="text" id="layer" value="' +
                      result.layer +
                      '" name="layer" class="form-control" placeholder="Type here">' +
                      '</div>' +
                      '</form>';

                    $(
                      '#gisNestedModal .modal .modal-dialog .modal-content .modal-body'
                    )[0].innerHTML = html;
                  })
                  .catch((error) => {
                    console.log(error);
                ...
                createNestedModal();
                $(
                  '#gisNestedModal .modal .modal-dialog .modal-content .modal-header .modal-title'
                )[0].innerText = 'Edit Layer';
                ...
                $('#gisNestedModalOk').click((e) => {
                  ...
                  fetch(ip_gis + 'gis/add_new_layer', config)
                    .then((response) => {
                      if (!response.ok) throw Error('There is something wrong');
                      return response.json();
                    })
            ...
            removeLayer: {
              ...
              action: function (obj) {
                ...
                $('#gisNestedModalOk').click((e) => {
                  ...
                  fetch(ip_gis + 'gis/delete_layer/' + node.id, config)
                    .then((response) => {
                      if (!response.ok) throw Error('There is something wrong');
                      return response.json();
                    })
          ...
          return {
            ...
            editCategory: {
              ...
              icon: 'lnr-pencil',
              action: () => {
                fetch(ip_gis + 'gis/get_category?id=' + node.id)
                  .then((response) => {
                    if (!response.ok) throw Error('There is something wrong');
                    return response.json();
                  })
                ...
                $('#gisNestedModalOk').click((e) => {
                  ...
                  fetch(ip_gis + 'gis/add_category', config)
                    .then((response) => {
                      if (!response.ok) throw Error('There is something wrong');
                      return response.json();
                    })
            ...
            removeCategory: {
              ...
              action: function (obj) {
                ...
                $('#gisNestedModalOk').click((e) => {
                  ...
                  fetch(ip_gis + 'gis/delete_category/' + node.id, config)
                    .then((response) => {
                      if (!response.ok) throw Error('There is something wrong');
                      return response.json();
                    })
...
input_geocode.addEventListener('input', (e) => {
  ...
  fetch(ip_gis + 'gis/suggest_geocode?input=' + input_geocode.value)
    .then((resp) => {
      if (!resp.ok) throw Error('There is something wrong');
      return resp.json();
    })
    .then(function (response) {
      if (response.status === 'OK') {
        response.predictions.forEach(function (v, k) {
          ...
          item.innerHTML =
            '<strong>' +
            v.description.substr(0, input_geocode.value.length) +
            '</strong>';
          item.innerHTML += v.description.substr(input_geocode.value.length);
          /*insert a input field that will hold the current array item's value:*/
          item.innerHTML +=
            "<input type='hidden' value='" + v.description + "'>";
...
function closeAutocomplete() {
  var existing = document.getElementById(
    input_geocode.id + ' autocomplete-list'
  );
  if (existing) input_geocode.parentNode.removeChild(existing);
}
  ...
  map.forEachFeatureAtPixel(evt.pixel, function (feature) {
    if (!drewSource.getFeatureById(feature.getId())) {
      ...
      let info = {
        ...
        '5': {
          column: 'Intersect Border',
          value: feature.get('intersect_border'),
        },
      };
  ...
  if (Array.isArray(parsed)) {
    parsed.forEach(function (v) {
      if (v.hasOwnProperty('id')) {
        ...
        if (v.status === 'Inside' && audio.paused) audio.play();
      }
    });
  }
};
...
function createModal() {
  ...
  modal.innerHTML =
    ...
    '                            <button type="button" class="btn btn-info" id="gisModalOk"></button>' +
    '                        </div>' +
    '                    </div>' +
    '                </div>' +
    '            </div>' +
...
function createNestedModal() {
  ...
  modal.innerHTML =
    ...
    '                            <button type="button" class="btn btn-info" id="gisNestedModalOk"></button>' +
    '                        </div>' +
    '                    </div>' +
    '                </div>' +
    '            </div>' +
